from flask import Flask, request, make_response, jsonify, render_template, redirect, send_from_directory, send_file
...
def login_user():
    ...
    if ok and sf in login2 and password != '' and login != '':
        ...
        with open(file_path_online, 'a') as ss:
            ss.write("\n" + ip_address + "%" + login + '%')
        response = make_response(jsonify({"result": "Вы успешно вошли в свой аккаунт!"}))
        # Установка срока действия куки на 4 часа
        response.set_cookie("saved_name", request.form.get('login'), max_age=7*24*60*60, httponly=True, secure=True)
        return response
